...
class MergePtSsIsInFoldersOptimized(object):
    """
    OPTIMIZED VERSION: Uses integer hashing instead of string concatenation.
    ~10x faster for large point clouds!
    """
    ...
    def merge(self):
        ...
        print(f"Found {len(list_of_matched_tupeles)} files to merge (OPTIMIZED method)")

        # merge the point cloud, semantic segmentation and instance segmentation
        for item in tqdm(list_of_matched_tupeles, desc="Merging files"):
            MergePtSsIsOptimized(
                point_cloud=item[0],
    ...
    def __call__(self, *args: Any, **kwds: Any) -> Any:
        if self.verbose:
            print("input_data_folder_path: {}".format(self.input_data_folder_path))
            print(
                "segmented_data_folder_path: {}".format(self.segmented_data_folder_path)
            )
            print("output_data_folder_path: {}".format(self.output_data_folder_path))
        self.merge()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Merge point cloud, semantic segmentation and instance segmentation in folders (OPTIMIZED)."
    )
    parser.add_argument(
        "-i", "--input_data_folder_path", help="Path to the input data folder."
    )
    parser.add_argument(
        "-s",
        "--segmented_data_folder_path",
    ...
    parser.add_argument(
        "-o", "--output_data_folder_path", help="Path to the output data folder."
    )
    parser.add_argument(
    ...
    # run the merge
    merge_pt_ss_is_in_folders = MergePtSsIsInFoldersOptimized(
